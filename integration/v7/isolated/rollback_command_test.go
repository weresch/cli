					...
					When("the user enters y", func() {
						...
						When("the strategy flag is provided without the max-in-flight flag", func() {
							...
							It("uses the given strategy to rollback without noting max-in-flight", func() {
								...
								Expect(session).To(HaveRollbackPrompt())
								Expect(session).To(HaveRollbackOutput(appName, orgName, spaceName, userName))
								Expect(session).Should(Say("Canary deployment currently PAUSED"))
								Expect(session).ShouldNot(Say("max-in-flight"))
								Expect(session).Should(Say("Please run `cf continue-deployment %s` to promote the canary deployment, or `cf cancel-deployment %s` to rollback to the previous version.", appName, appName))
								Expect(session).To(Say("OK"))
							})
						})
